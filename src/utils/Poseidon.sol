...
/// @author Modified from https://github.com/vimwitch/poseidon-solidity/blob/main/contracts/PoseidonT2.sol
library PoseidonT2 {
    ...
    function hash(uint256 input) internal pure returns (uint256 out) {
        uint256 F = 21888242871839275222246405745257275088548364400416034343698204186575808495617;

        if (input >= F) revert InvalidFieldElement();

        assembly {
            let state1 := add(input, 0x0c0356530896eec42a97ed937f3135cfc5142b3ae405b8343c1d83ffa604cb81)
            ...
            out := mod(add(mulmod(scratch0, M00, F), mulmod(scratch1, M10, F)), F)
        }
    }
}
...
library PoseidonT3 {
    ...
    function hash(uint256 input1, uint256 input2) internal pure returns (uint256 out) {
        ...
        if (input1 >= F || input2 >= F) revert InvalidFieldElement();
        ...
        assembly {
            ...
            let state1 := add(input1, 0x00f1445235f2148c5986587169fc1bcd887b08d4d00868df5696fff40956e864)
            let state2 := add(input2, 0x08dff3487e8ac99e1f29a058d0fa80b930c728730b7ab36ce879f3890ecf73f5)
            let scratch0 := mulmod(state1, state1, F)
            state1 := mulmod(mulmod(scratch0, scratch0, F), state1, F)
