...
/// @author Modified from https://github.com/vimwitch/poseidon-solidity/blob/main/contracts/PoseidonT3.sol
library Poseidon {
    ...
    function hash(uint256 input1, uint256 input2) internal pure returns (uint256 out) {
        assembly {
            ...
            let state1 := add(mod(input1, F), 0x00f1445235f2148c5986587169fc1bcd887b08d4d00868df5696fff40956e864)
            let state2 := add(mod(input2, F), 0x08dff3487e8ac99e1f29a058d0fa80b930c728730b7ab36ce879f3890ecf73f5)
            ...
            out := mod(add(add(mulmod(state0, M00, F), mulmod(state1, M10, F)), mulmod(state2, M20, F)), F)
        }
    }
}
